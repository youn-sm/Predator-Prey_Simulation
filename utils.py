...
def run_simulation(FPS):
    ...
    while running:
        ...
        for idx, predator in enumerate(predators):
            ...
            predator.time_without_food += 1 / FPS
            # 포식자 화면에 생성하고 움직이기
            predator.move()
            ...
            #사냥 가능 범위 안이면 쫓아가기
                ...
                # 포식자가 피식자 보면 가속
                # predator.x += predator.direction[0] * PREDATOR_SPEED
                # predator.y += predator.direction[1] * PREDATOR_SPEED
                distance = np.linalg.norm(np.array([predator.x - prey.x, predator.y - prey.y]))
        ...
        for prey in preys:
            ...
            if prey.can_reproduce(current_time):
                ...
                new_preys.direction = prey.direction * new_preys_speed / np.linalg.norm(prey.direction)
        ...
        # 피식자가 다 죽으면 시뮬레이션 멈추기
        #if SIMULATION_DURATION > 0 and current_time - start_time >= SIMULATION_DURATION:
        #   running = False
