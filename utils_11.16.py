...
half_time_steps = []
half_prey_population = []
half_predator_population = []
...
def update_half_population_data(population_measure_time, prey_count, predator_count, preys):
    half_time_steps.append(population_measure_time)
    half_prey_population.append(prey_count)
    half_predator_population.append(predator_count)
    prey_speeds = [i.speed for i in preys]
    if len(prey_speeds):
        average_preys_speeds_every_half_second.append(sum(prey_speeds) / len(prey_speeds))
    else:
        average_preys_speeds_every_half_second.append(0)
...
def scatter():
    plt.subplot(3, 1, 3)
    data = average_preys_speeds_every_half_second[20:]
    t = range(10, len(average_preys_speeds_every_half_second))
...
def run_simulation(FPS):
    ...
    while running:
        ...
        if int(current_time) > len(average_preys_speeds_every_half_second):
            prey_speeds = [i.speed for i in preys]
            if len(prey_speeds):
                average_speed = sum(prey_speeds) / len(prey_speeds)
                average_preys_speeds_every_half_second.append(average_speed)
        ...
        clock.tick(FPS)
        if int(start_time) >= 30:
            print(average_preys_speeds_every_half_second)
