...
class Prey:
    ...
    def __init__(self):
        ...
        self.direction = np.array([random.uniform(-1, 1), random.uniform(-1, 1)])
        self.direction /= np.linalg.norm(self.direction)
        self.reproduction_delay = random.randint(5, 20 )
        self.last_reproduction_time = 0
        self.current_time = 0
    ...
    # 피식자 복제하기
    def can_reproduce(self, current_time):
        if current_time - self.last_reproduction_time >= self.reproduction_delay and random.randint(0,5) < 2:
            return True
        return False
